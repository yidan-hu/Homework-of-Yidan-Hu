...
L=20; n=512; N=n*n;
x2=linspace(-L/2,L/2,n+1); x=x2(1:n); y=x;%512份从-10到10
kx=(2*pi/L)*[0:(n/2-1) -n/2:-1]; ky=kx;%512份，弧度从0度到-1/256*2pi度
...
u(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*cos(m*angle(X+1i*Y)-(sqrt(X.^2+Y.^2)));
v(:,:,1)=tanh(sqrt(X.^2+Y.^2)).*sin(m*angle(X+1i*Y)-(sqrt(X.^2+Y.^2)));
